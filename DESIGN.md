...
colors:
  ...
  red-bright: "#ff525b"
  btn-top: "#e01d2b"
  btn-hover-top: "#e62332"
  btn-hover-bottom: "#c30f20"
  red-shade: "#b90c1b"
  red-deep: "#a80d18"
  red-wash: "#fff0f1"
  ink: "#0d0e0f"
  ...
  gold-number: "#df1726"
  gold-hover: "#ff525b"
  gold-cream-hover: "#ff525b"
  gold-tint: "#fff0f1"
  gold-tint-warm: "#fff0f1"
  gold-footer: "#ff525b"
  gold-pin-ink: "#ffffff"
  on-gold: "#ffffff"
typography:
...
components:
  ...
  button-cream:
    ...
    height: "44px"
  button-cream-hover:
    backgroundColor: "{colors.gold-cream-hover}"
  button-dark:
  ...
  promotion-card-mobile:
    rounded: "{rounded.banner-mobile}"
  hero-promotion-card:
    backgroundColor: "{colors.ink-banner}"
    rounded: "{rounded.media}"
  hero-promotion-card-featured:
    backgroundColor: "{colors.ink-banner}"
    rounded: "{rounded.banner-featured}"
  chip-tag:
    backgroundColor: "{colors.surface-tag-alt}"
    textColor: "{colors.label-quiet}"
    rounded: "{rounded.pill}"
    padding: "5px 8px 4px"
  chip-filter-active:
    backgroundColor: "{colors.ink}"
  ...
  nav-link-active:
    textColor: "#ffffff"
  breadcrumb-bar:
    backgroundColor: "{colors.surface-stat}"
    textColor: "{colors.text-crumb}"
    height: "56px"
  spec-panel:
    backgroundColor: "{colors.card}"
    textColor: "{colors.ink}"
...
- CMS artwork is framed, never overlaid — the frame carries the geometry, the art carries the message.
...
**Character:** Space Grotesk does all the work — it is technical enough to set a spec table and humane enough to set a lead paragraph, and its wide weight range (400 through 750, all real on the variable face) makes a whole hierarchy possible in one family. Instrument Serif appears exactly once per page at most, and currently in exactly one place in the whole build: the founding year on Sobre's história band. It is the system's only ornament, and its scarcity is what makes it register.

### Hierarchy
- **Display** (580–650, `clamp(39px … 44px, 4.2–5.3vw, 60–72px)`, line-height .98–1.06, tracking -.045 to -.052em): Page `h1` only, one per surface. Always `text-wrap: balance` and always capped at 12–14ch so it breaks into two or three deliberate lines. The clamp floor and ceiling are per-surface, not global: home 48→60px, catálogo 56px fixed (mobile clamps 40→54), produto 42→60px, sobre 44→66px, localização 44→72px.
- **Display Serif** (400, `clamp(62px, 7vw, 92px)`, line-height .88, tracking -.03em): The single ornamental number. One per page, maximum.
- **Headline** (600, `clamp(28–36px, 3–3.8vw, 38–50px)`, line-height 1.04–1.08, tracking -.04 to -.045em): Band titles (`h2`) on the newer surfaces. Capped at 15–19ch.
- **Headline (fixed)** (600, 44 / 42 / 40px, line-height 1.05, tracking -.04em): The home bands still pin fixed sizes, stepping to 38px at 800px and 34px at 520px. This is the older register; a new band should use the clamp.
- **Title** (650, 18–24px, line-height 1.1–1.25, tracking -.02 to -.035em): Card names, capability headings, the map card's `h2` (24px), the spec panel's `h2` (20px).
- **Body** (400, 15–17px, line-height 1.6–1.68): Leads and prose, in Cinza Texto on paper or white-at-64–70% on ink. Always capped by measure — 48ch for a product lead, 52ch for a closing paragraph, 55–58ch for a section lead, 62–68ch for long prose.
- **Body Small** (400, 12–14px, line-height 1.42–1.56): Card descriptions, notes, breadcrumb links (11–12px), footnotes under a form.
- **Label** (700, 11–12px, tracking .1–.16em, uppercase): Eyebrows, `dt` in a data list, sidebar group legends, footer group titles, the mobile filter legend (12px / .16em).
- **Micro** (700–750, 9–10px, tracking .08–.13em, uppercase): Card footers, result counts, badges, stat labels, filter-state captions. The smallest type in the system; never used for anything a visitor must read to act.

**Numerals.** Step numerals (`01`, `02`, `03`) are set at 12px / 750 with `font-variant-numeral: tabular-nums` on cards, and at 35px / 700 in the home process glass tiles. Prices are 24px / 620, tabular. Numbers are never set in the serif except the single ornamental year.

### Named Rules
**The Tracking Follows Size Rule.** Tracking is a function of size and runs monotonically: -.045 to -.052em at display, -.04em at headline, -.02 to -.035em at title, normal at body, then *positive* — .08em to .16em — the moment type goes uppercase and under 13px. A 44px heading at normal tracking, or an 11px uppercase label at normal tracking, are both immediately wrong.

**The Measure Rule.** Every text block declares a cap in `ch`, not in pixels: 12–19ch for headings, 48–68ch for prose. A paragraph without a measure cap is unfinished. (The one place the cap is on the *element* rather than the text is the breadcrumb's current page: 42ch desktop, 22ch mobile, with an ellipsis.)
...
**Container.** One container, everywhere: `.lm-container` is `width: min(calc(100% - 48px), var(--lm-grid-width))` centred — a 1216px grid with 24px gutters, dropping to 20px gutters (`--lm-mobile-gutter`) below 800px. Bands are full-bleed; only their content is contained. There is no second max-width nested inside a container anywhere in the build.

**Band stack.** Every page is a vertical stack of full-bleed bands, each with its own substance, alternating so no two adjacent bands share a background.
- **Home:** ink hero (photographic, with the banner pair floated over it) → paper catálogo → deep-ink unidades → paper-2 banner faixa 1 → paper grupo → photographic processo → paper-2 banner faixa 2 → paper contato → ink footer.
- **Catálogo:** paper hero → paper catálogo (sidebar + grid) → ink footer.
- **Produto:** breadcrumb topbar (`#efe8dc`) → paper hero (gallery + sticky summary) → paper-2 informação → paper relacionados → ink unidades → ink footer.
- **Sobre:** paper hero → paper-2 capacidades → paper projetos → paper-2 história → paper equipe → paper-2 método → ink CTA.
- **Localização:** paper hero (with the map stage inside it) → paper-2 cobertura → paper visita → ink footer.

**Vertical rhythm.** Desktop band padding runs 84–90px on Sobre and the product page, 72–84px on Localização, 62–64px on the home bands, and 32–40px on the banner bands (which carry only artwork, so they need less air). Every one of them collapses to 56px below 800px. Inside a band the ladder is 8 / 12 / 16 / 20 / 24 / 32 / 40 / 48 / 64: eyebrow → 14–22px → heading → 16–24px → lead → 24–30px → action; a ruled list's rows sit at 14–24px of padding with a 1px rule between.

**Column grammar.** Two columns, deliberately unequal, `align-items: start` or `center`, gap 44–82px, both sides expressed as `minmax()` so neither collapses: a reading column at `minmax(0, .9–1.25fr)` and an evidence column at `minmax(330–460px, .72–1.18fr)`. Grids: 4 columns for home products, catalogue recommendations and Sobre's projects; 3 for the catalogue results, the team roster, the process rows and the unit rows; **2 for the promotional banner grid**; `250px` sidebar + `minmax(0, 1fr)` for the catalogue layout.

**Sticky offsets.** Sticky elements park at `top: 92px` (catalogue sidebar), `96px` (product summary) or `100px` (Localização's copy column); anchor targets carry `scroll-margin-top` of 96px on desktop, 80–72px on mobile — matched to the 68px/64px header plus air.

**Breakpoints.** Three, and only three: **1100px** (columns tighten, 4-up becomes 2-up or 3-up, the hero banner pair is hidden, the nav gap tightens to 18px), **800px** (everything goes single column; grids become horizontal snap carousels; the nav becomes a blurred dropdown behind a 44px toggle; the catalogue sidebar becomes a bottom sheet; the banner grid becomes one per row), **520px** (label/value pairs stack vertically, buttons go full-width, carousel cards go edge-to-edge). Two behavioural queries sit alongside them: `@media (hover: none)` removes every hover transform and shadow, and the reduced-motion pair described under Components → Motion.
...
**The Closing Ink Rule.** A page ends on ink. In practice this is the site footer, which is ink on every page; two surfaces additionally close their own content on an ink band immediately above it — Sobre on `sobre-cta` and Produto on `product-detail__units`. Home's contact band and Localização's visita band close on paper and lean on the footer alone. Prefer the explicit ink close on a new surface that carries a primary action.

**The No Empty Half Rule.** When a two-column band loses its evidence half — no map, no photo, no image in the CMS — the band collapses to one column and the reading column widens (the `is-solo` modifier, live on `location-visit__grid`). It never ships with half a section blank. This is a layout obligation, not a nicety: content is CMS-driven and the empty case is the normal case.

**The Frame-Not-Canvas Rule.** A CMS-supplied banner is framed, never composed on. The system supplies the aspect ratio, radius, ring, hover and focus; the artwork supplies everything inside. No heading, kicker, gradient scrim or button is laid over promotional artwork — if a message needs type, it belongs in the art or in a real band.

## Elevation & Depth

The system is **flat on paper and glassy on ink**. Nothing on a paper band casts a shadow at rest — separation is done with a hairline (`1px solid` Filete as a border, or `inset 0 0 0 1px` as a ring that costs no layout) and with a half-step of tone (Cartão on Papel). Shadow is a *response*: it appears when the pointer arrives and disappears with it, and `@media (hover: none)` removes it entirely.

Two exceptions to flat-on-paper are real and deliberate: a **large media object** — the product gallery, Sobre's hero visual, the map stage, a section image — carries a soft ambient shadow at rest because it is a physical artefact laid on the page rather than a card printed into it; and a **promotional banner** carries one for the same reason. Both stay in the 7–15% opacity range at a 18–70px blur.
...
- **Media seat** (`0 18px 48px … 0 24px 70px rgb(39 31 21 / 7–15%)` + a `1px solid rgb(0 0 0 / 10%)` inset outline): Large images, the product gallery, the map stage.
- **Panel seat** (`inset 0 0 0 1px rgb(0 0 0 / 5%), 0 10px 34px rgb(39 31 21 / 6–7%)`): The spec panel and the paper step cards. A softer relative of the hairline ring, for panels large enough that a bare hairline reads thin.
- **Bottom sheet** (`0 -18px 60px rgb(0 0 0 / 24%)`): Upward, because the sheet comes from below.

### Named Rules
**The Hairline-At-Rest Rule.** On paper, a *card* at rest is separated by a 1px line and a half-step of tone. Shadow at rest is reserved for large media objects, framed artwork, and things that genuinely float (the bottom sheet, the glass chrome). A resting drop shadow under a paper *card* is still out of world.

**The Glass Needs Ground Rule.** `backdrop-filter` is only legitimate where there is something to blur: over photography, over ink, or over a map. Glass on a paper band is a defect, not a variant.

## Shapes

Radius is a function of surface area, and it climbs a clear ladder: **pill** (999px) for anything that holds a word — buttons, filter chips, tags, badges, the carousel progress bar, the map credit; **circle** (50%) for anything that holds a mark — arrow discs (32/34/40/42px), the mobile toggle, the map pin (44px), the sheet close (48px), the eyebrow dot (5px) and the capability dot (7px); **9–14px** for controls and small objects — inputs (9px), the sidebar clear button and active-filter button (10px), the gallery thumbnail (10px), search fields (12px), avatars and the filter-state block (14px); **16px** for cards and rows; **18–24px** for media, panels and framed artwork — the hero banner (18px, 24px when featured), the história image (20px), the promotional banner (20px, 16px on mobile), the spec panel and step cards (22–24px); **26–40px** for the largest objects — the map card (26px), the contact card and form (34px, stepping to 28px then 24px), the bottom sheet (32px on the top corners only), the product gallery (30px), Sobre's hero visual (36px), the catalogue hero image (40px).
...
**The Pill For Words Rule.** Anything that contains a word and can be pressed is a pill (999px). Anything that contains only a mark is a circle. There is no in-between rounded rectangle for actions — the one exception is the 56px mobile filter trigger and the sidebar's utility buttons, which are square-ish (10–18px) because they hold an icon inside a grid of fields, not a word inside a row of actions.

**The Aspect Ratio Rule.** Any container that receives an unknown image declares an `aspect-ratio` (or a fixed height) and clips. A grid of artwork whose heights are set by the artwork will never align, and CMS art is by definition unknown.
...
- **Glass card** — over photography or ink only. Diagonal translucent gradient, 18px backdrop blur, four-layer shadow, a 35px tabular numeral and an arrow disc in the header row, then a hairline rule, then title and copy.
...
### Icons
Traced SVG on a 24 viewBox, `fill: none`, `stroke: currentColor`, `stroke-width: 1.7` (1.8 on the mobile filter triggers), round caps and joins. Rendered at 18–27px. Every icon is `aria-hidden` unless it is the only content of a labelled control; its meaning otherwise lives in the surrounding text.

### Overlays
- **Bottom sheet** (`.mobile-filter-sheet`): below 800px only, on both the home catalogue and the catalogue page. Top corners at 32px, a 54px drag handle, a 30px title with a 48px circular close, up to `min(86svh, 720px)` tall (`88svh / 760px` on the catalogue), respecting `env(safe-area-inset-bottom)`, sliding up over 240ms behind a 54%-opaque backdrop that fades over 180ms. It is `role="dialog" aria-modal="true"` and is `visibility: hidden` + `pointer-events: none` when closed, with the visibility transition delayed so the slide-out is visible.

There is no product detail modal. Selecting a product navigates.

### Motion
- **Durations:** 100ms press · 150–180ms colour and state · 200–260ms lift, panel and sheet · 400–650ms image scale · 620–880ms entrance.
- **Easings:** `ease` for colour; `cubic-bezier(.2, .65, .3, 1)` for lift, carousel, sheet and banner; `cubic-bezier(.2, .7, .3, 1)` for cards; `cubic-bezier(.16, .84, .44, 1)` for entrances.
- **Transform budget:** hover lifts are `translateY(-5px)` to `translateY(-6px)` with scale ≤ 1.035; image scale inside a frame is ≤ 1.06.

**The Press Rule.** Every pressable thing scales to `.96` on `:active` over 100ms — buttons, filter chips, cards, thumbnails, sheet controls, linked banners. It is the system's single tactile signature and it is applied without exception. (The band banner presses to `translateY(-2px) scale(.995)` instead, because it is already lifted when pressed.)

**The One Animated Moment Rule.** A page gets one entrance animation and no more. It is `lm-sobe` (14px rise + fade, 620ms) staggered in 80ms steps to a 240ms ceiling, currently on Sobre's stat trio and Localização's coverage cards. Both live inside `@media (prefers-reduced-motion: no-preference)` — opt-in, so motion is never the default. A global `prefers-reduced-motion: reduce` block additionally clamps every animation and transition in the build to .01ms and disables smooth scrolling.
...
- **Interactivity is never implied by styling alone**: a link-less banner is a `<div>` and has no hover, lift or press.
- **Decorative marks are `aria-hidden`** (eyebrow dots, arrows, separators, icons, the duplicated marquee track) and decorative images carry `alt=""` inside a wrapper with `role="img"` and an `aria-label`.
- **`lang="pt-BR"`** on the document; all copy, labels and admin vocabulary in pt-BR. Landmarks are labelled in pt-BR too (`aria-label="Você está aqui"`, `"Navegação principal"`, `"Destaques da LM"`).
...
### Do:
- **Do** compose a new page as a stack of full-bleed bands, alternating substance, and close a surface carrying a primary action on an ink band.
...
- **Do** let a horizontal trail of items scroll rather than wrap when wrapping would change the height of a fixed band.
